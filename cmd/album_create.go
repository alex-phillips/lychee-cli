...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if cmd.Flag("parent").Value.String() != "" {
			parent := api.GetAlbumByName(cmd.Flag("parent").Value.String())
			if parent != nil {
				parentID = parent.GetID()
			}
		}

		id := api.CreateAlbum(args[0], parentID)
		api.GetAlbumInfo(id)
	},
}

func init() {
	albumCreateCmd.PersistentFlags().String("parent", "", "Parent album name")
}
